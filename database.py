...
def init_lid_tables():
    ...
    c.execute("""
    CREATE TABLE IF NOT EXISTS lid_users (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
    ...
    c.execute("""
    CREATE TABLE IF NOT EXISTS lid_flows (
        id          INTEGER PRIMARY KEY AUTOINCREMENT,
...
def lid_create_user(telegram_id, name, username, bot_id, source='direct'):
    conn = get_db()
    conn.execute(
        "INSERT OR IGNORE INTO lid_users (telegram_id, bot_id, name, username, source) VALUES (?,?,?,?,?)",
        (telegram_id, bot_id, name, username, source)
    )
    # Click hisobi
    if source != 'direct':
        conn.execute(
            "UPDATE lid_links SET clicks = clicks + 1 WHERE source=?", (source,)
        )
    conn.commit()
    conn.close()


def lid_get_user(telegram_id, bot_id):
    conn = get_db()
    row = conn.execute(
        "SELECT * FROM lid_users WHERE telegram_id=? AND bot_id=?",
        (telegram_id, bot_id)
    ).fetchone()
    conn.close()
    return row
...
def lid_increment(telegram_id, bot_id):
    conn = get_db()
    conn.execute(
        "UPDATE lid_users SET msg_count = msg_count + 1 WHERE telegram_id=? AND bot_id=?",
        (telegram_id, bot_id)
    )
    conn.commit()
    conn.close()
...
def get_setting_for_bot(bot_id, key):
    conn = get_db()
    row = conn.execute(
...
def set_setting_for_bot(bot_id, key, value):
    conn = get_db()
    conn.execute(
        "INSERT OR REPLACE INTO bot_settings (bot_id, key, value) VALUES (?,?,?)",
        (bot_id, key, value)
    )
    conn.commit()
    conn.close()
...
def get_lid_stats(bot_id):
    conn = get_db()
    total = conn.execute(
        "SELECT COUNT(*) as c FROM lid_users WHERE bot_id=?", (bot_id,)
    ).fetchone()['c']
    links = conn.execute(
        "SELECT source, name, clicks, (SELECT COUNT(*) FROM lid_users WHERE source=l.source AND bot_id=?) as lids FROM lid_links l ORDER BY clicks DESC",
        (bot_id,)
    ).fetchall()
    conn.close()
    return total, [dict(l) for l in links]

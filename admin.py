...
<div class="sidebar">
  ...
  <div style="padding:8px 20px;font-size:10px;font-weight:700;color:#7a8398;text-transform:uppercase;letter-spacing:.6px;margin-top:8px">Lid Magnit Bot</div>
  <a href="/admin/lid" class="nav-item {% if page=='lid' %}active{% endif %}"><i class="ti ti-chart-bar"></i> Dashboard</a>
  <a href="/admin/lid/links/new" class="nav-item {% if page=='lid_link' %}active{% endif %}"><i class="ti ti-link"></i> Linklar</a>
  <a href="/admin/lid/flows/new" class="nav-item {% if page=='lid_flow' %}active{% endif %}"><i class="ti ti-route"></i> Flowlar</a>
  <a href="/admin/lid/settings" class="nav-item {% if page=='lid_settings' %}active{% endif %}"><i class="ti ti-settings"></i> Sozlamalar</a>
...
# ===== LID BOT ADMIN ROUTES =====

@app.route('/admin/lid')
@login_required
def lid_dashboard():
    from database import get_lid_stats, get_db
    total, links = get_lid_stats('lid')
    conn = get_db()
    flows = conn.execute("SELECT * FROM lid_flows ORDER BY created_at DESC").fetchall()
    conn.close()
    ...
    for l in links:
        ...
        links_html += f"""
        <tr>
          <td><code>t.me/bot?start={l['source']}</code></td>
          <td>{l['name'] or '—'}</td>
          <td>{l['clicks']}</td>
          <td style="color:var(--text-success);font-weight:600">{l['lids']} ({conv}%)</td>
          <td>
            <a href="/admin/lid/links/edit/{l['source']}" class="btn btn-outline btn-sm">Tahrirlash</a>
          </td>
        </tr>"""
    ...
    for f in flows:
        ...
        <tr>
          ...
          <td>{f['name'] or '—'}</td>
          <td>{'✅ Faol' if f['active'] else '⏸ To\'xtatilgan'}</td>
          <td><a href="/admin/lid/flows/edit/{f['flow_id']}" class="btn btn-outline btn-sm">Tahrirlash</a></td>
        </tr>"""

    content = f"""
<div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px">
  <div class="page-title" style="margin:0">📨 Lid Magnit Bot</div>
  <div style="display:flex;gap:8px">
    <a href="/admin/lid/links/new" class="btn btn-outline">+ Yangi link</a>
    <a href="/admin/lid/flows/new" class="btn btn-primary">+ Yangi flow</a>
  </div>
</div>
<div class="kpi-grid">
  <div class="kpi"><div class="num">{total}</div><div class="label">Jami lidlar</div></div>
  <div class="kpi blue"><div class="num">{len(links)}</div><div class="label">Tracking linklar</div></div>
  <div class="kpi green"><div class="num">{len(flows)}</div><div class="label">Flowlar</div></div>
</div>

<div class="card">
  <div class="card-header">Tracking linklar (UTM)</div>
  <div class="card-body" style="padding:0">
    <table>
      <thead><tr><th>Link</th><th>Nomi</th><th>Bosildi</th><th>Lid bo'ldi</th><th></th></tr></thead>
      <tbody>{links_html or '<tr><td colspan="5" style="text-align:center;color:#7a8398;padding:16px">Hali link yo\'q</td></tr>'}</tbody>
    </table>
  </div>
</div>

<div class="card">
  <div class="card-header">Flowlar</div>
  <div class="card-body" style="padding:0">
    <table>
      <thead><tr><th>Flow ID</th><th>Nomi</th><th>Holati</th><th></th></tr></thead>
      <tbody>{flows_html or '<tr><td colspan="4" style="text-align:center;color:#7a8398;padding:16px">Hali flow yo\'q</td></tr>'}</tbody>
    </table>
  </div>
</div>
"""
    return render_template_string(TEMPLATE.replace("{% block content %}{% endblock %}", content),
                                  logged_in=True, page='lid')


@app.route('/admin/lid/links/new', methods=['GET', 'POST'])
@login_required
def lid_link_new():
    msg = None
    if request.method == 'POST':
        ...
        # Media sozlamalari
        from database import set_setting_for_bot
        ...
        conn = get_db()
        conn.execute(
            "INSERT OR REPLACE INTO lid_links (source, name, flow_id) VALUES (?,?,?)",
            (source, name, flow_id)
        )
        conn.commit()
        conn.close()
        msg = f"✅ Link yaratildi: t.me/bot?start={source}"

    conn = get_db()
    flows = conn.execute("SELECT flow_id, name FROM lid_flows WHERE active=1").fetchall()
    conn.close()

    flow_opts = '<option value="">Flow yo\'q (oddiy xabar)</option>' + "".join(
        f'<option value="{f["flow_id"]}">{f["name"] or f["flow_id"]}</option>'
        for f in flows
    )
    ...
    media_opts = "".join(
        ...
        for v, l in [('','Faqat matn'),('photo','🖼 Rasm'),('video','🎬 Video'),
                     ('video_note','⭕ Dumaloq'),('audio','🎵 Audio'),
                     ('voice','🎤 Ovozli'),('document','📄 PDF/Fayl')]
    )
...
<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
  <a href="/admin/lid" style="color:var(--text-muted);font-size:20px">←</a>
  <div class="page-title" style="margin:0">Yangi tracking link</div>
</div>
{msg_html}
<div class="card">
<div class="card-body">
<form method="POST">
  <div class="two-col">
    <div class="form-group"><label>Link ID (lotin harflar, _ bilan)</label><input name="source" placeholder="instagram_post1" required></div>
    <div class="form-group"><label>Nomi (esda qolish uchun)</label><input name="name" placeholder="Instagram post #1"></div>
  </div>
  <div class="form-group"><label>Flow (ixtiyoriy)</label><select name="flow_id">{flow_opts}</select></div>
  <hr style="margin:14px 0;border:none;border-top:1px solid #e8ecf6">
  <p style="font-size:12px;color:#7a8398;margin-bottom:12px">Agar flow yo'q bo'lsa — quyidagi xabar yuboriladi:</p>
  <div class="form-group"><label>Xabar matni</label><textarea name="text" rows="3" placeholder="Salom! Mana sizning link orqali materialing 🎁"></textarea></div>
  <div class="two-col">
    <div class="form-group"><label>Media turi</label><select name="media_type">{media_opts}</select></div>
    <div class="form-group"><label>File ID (Telegramdan)</label><input name="media_file_id" placeholder="AgACAgI..."></div>
  </div>
  <button type="submit" class="btn btn-primary">Saqlash</button>
</form>
</div>
...
@app.route('/admin/lid/flows/new', methods=['GET', 'POST'])
@login_required
def lid_flow_new():
    import json
    msg = None
    if request.method == 'POST':
        flow_id = request.form.get('flow_id', '').strip().replace(' ', '_')
        name = request.form.get('name', '')
        ...
        conn = get_db()
        conn.execute(
            "INSERT OR REPLACE INTO lid_flows (flow_id, name, steps) VALUES (?,?,?)",
            (flow_id, name, steps_json)
        )
        conn.commit()
        conn.close()
        msg = f"✅ Flow '{name}' yaratildi!"
...
<div style="display:flex;align-items:center;gap:12px;margin-bottom:20px">
  <a href="/admin/lid" style="color:var(--text-muted);font-size:20px">←</a>
  <div class="page-title" style="margin:0">Yangi flow</div>
</div>
{msg_html}
<div class="card">
<div class="card-body">
<form method="POST">
  <div class="two-col">
    <div class="form-group"><label>Flow ID</label><input name="flow_id" placeholder="video1_flow" required></div>
    <div class="form-group"><label>Nomi</label><input name="name" placeholder="Video #1 flow"></div>
  </div>

  <div class="form-group">
    <label>Flow qadamlari</label>
    <div id="steps-list" style="margin-bottom:10px"></div>
    <button type="button" onclick="addStep()" class="btn btn-outline btn-sm">+ Qadam qo'shish</button>
  </div>

  <input type="hidden" name="steps_json" id="steps_json">
  <button type="submit" class="btn btn-primary" onclick="saveSteps()">Saqlash</button>
</form>
</div>
</div>
...
function addStep(){{
  ...
  steps.push({{delay:0,text:'',media_type:'',media_file_id:''}});
  renderSteps();
}}
...
function renderSteps(){{
  ...
  container.innerHTML = steps.map((s,i) => `
    <div style="background:var(--bg);border:1px solid #e8ecf6;border-radius:10px;padding:14px;margin-bottom:10px">
      <div style="display:flex;justify-content:space-between;margin-bottom:10px">
        <b style="font-size:12px">Qadam ${{i+1}}</b>
        <button type="button" onclick="removeStep(${{i}})" style="font-size:11px;color:#e42a3b;background:none;border:none;cursor:pointer">O'chirish</button>
      </div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:8px">
        <div><label style="font-size:10px;color:#7a8398;display:block;margin-bottom:4px">Kutish (soniya)</label>
          <input type="number" value="${{s.delay}}" onchange="steps[${{i}}].delay=parseInt(this.value)||0" style="width:100%;font-size:12px"></div>
        <div><label style="font-size:10px;color:#7a8398;display:block;margin-bottom:4px">Media turi</label>
          <select onchange="steps[${{i}}].media_type=this.value" style="width:100%;font-size:12px">
            <option value="">Faqat matn</option>
            <option value="photo">🖼 Rasm</option>
            <option value="video">🎬 Video</option>
            <option value="video_note">⭕ Dumaloq</option>
            <option value="audio">🎵 Audio</option>
            <option value="voice">🎤 Ovozli</option>
            <option value="document">📄 PDF</option>
          </select></div>
      </div>
      <div style="margin-bottom:8px"><label style="font-size:10px;color:#7a8398;display:block;margin-bottom:4px">File ID (ixtiyoriy)</label>
        <input value="${{s.media_file_id}}" onchange="steps[${{i}}].media_file_id=this.value" placeholder="AgACBg..." style="width:100%;font-size:12px"></div>
      <div><label style="font-size:10px;color:#7a8398;display:block;margin-bottom:4px">Matn / Caption</label>
        <textarea onchange="steps[${{i}}].text=this.value" rows="2" style="width:100%;font-size:12px;resize:none">${{s.text}}</textarea></div>
    </div>
  `).join('');
}}
...
def lid_settings():
    ...
    msg = None
    if request.method == 'POST':
        set_setting_for_bot('lid', 'welcome_text', request.form.get('welcome_text', ''))
        set_setting_for_bot('lid', 'welcome_media_type', request.form.get('welcome_media_type', ''))
        set_setting_for_bot('lid', 'welcome_media_file_id', request.form.get('welcome_media_file_id', ''))
        set_setting_for_bot('lid', 'auto_reply_text', request.form.get('auto_reply_text', ''))
        msg = "✅ Saqlandi!"
    ...
    media_opts = lambda cur: "".join(
        ...
        for v, l in [('','Faqat matn'),('photo','🖼 Rasm'),('video','🎬 Video'),
                     ('video_note','⭕ Dumaloq'),('audio','🎵 Audio'),
                     ('voice','🎤 Ovozli'),('document','📄 PDF')]
    )

    msg_html = f'<div class="alert alert-success">{msg}</div>' if msg else ''

    content = f"""
<div class="page-title">Lid Magnit Bot — Sozlamalar</div>
{msg_html}
<div class="card">
<div class="card-body">
<form method="POST">
  <div style="font-size:13px;font-weight:600;margin-bottom:10px">Salomlashuv xabari (/start)</div>
  <div class="form-group"><label>Matn</label>
    <textarea name="welcome_text" rows="3">{gs('welcome_text') or 'Assalomu alaykum! 👋\n\nZamira Turganbayeva botiga yozdingiz.\nSizga qanday yordam bera olaman?'}</textarea></div>
  <div class="two-col">
    <div class="form-group"><label>Media turi</label><select name="welcome_media_type">{media_opts(gs('welcome_media_type'))}</select></div>
    <div class="form-group"><label>File ID</label><input name="welcome_media_file_id" value="{gs('welcome_media_file_id')}" placeholder="Telegram File ID"></div>
  </div>
  <hr style="margin:14px 0;border:none;border-top:1px solid #e8ecf6">
  <div style="font-size:13px;font-weight:600;margin-bottom:10px">Avtomatik javob</div>
  <div class="form-group"><label>User xabar yuborganda avtomatik javob (bo'sh qoldirsangiz — faqat adminga forward)</label>
    <input name="auto_reply_text" value="{gs('auto_reply_text')}" placeholder="Xabaringiz qabul qilindi, tez orada javob beramiz ⏰"></div>
  <button type="submit" class="btn btn-primary">Saqlash</button>
</form>
</div>
</div>
